...
def soma(u, v):
    # Implementa a soma dos vetores aqui
    return np.array([u[i] + v[i] for i in range(3)], dtype="complex128")


def produto(beta, u):
    # Implementa o produto aqui
    mod_beta = np.abs(beta)  # Módulo de beta
    return np.array([mod_beta * u[i] for i in range(3)], dtype="complex128")
...
def verifica_soma(u, v):
    ...
    resultado = soma(u, v)
    return np.all(
        np.iscomplexobj(resultado)
    )  # Verifica se todos os elementos são complexos
...
def inverso(v):
    return np.array([-v[i] for i in range(3)], dtype="complex128")
...
def verifica_comutatividade(u, v):
    ...
    Verifica se u + v = v + u
    """
    return np.all(soma(u, v) == soma(v, u))
...
def verifica_associatividade(u, v, w):
    """
    Verifica se u + (v + w) = (u + v) + w
    """
    return np.all(soma(u, soma(v, w)) == soma(soma(u, v), w))
...
def verifica_distributividade_1(beta, u, v):
    ...
    """
    return np.all(produto(beta, soma(u, v)) == soma(produto(beta, u), produto(beta, v)))
...
def verifica_distributividade_2(beta, gamma, u):
    ...
    """
    return np.all(produto(beta + gamma, u) == soma(produto(beta, u), produto(gamma, u)))
...
def verifica_distributividade_3(beta, gamma, u):
    ...
    """
    return np.all(produto(beta, produto(gamma, u)) == produto(beta * gamma, u))
